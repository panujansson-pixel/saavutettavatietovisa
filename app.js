/* app.js
Accessible quiz SPA (no backend required). Uses SpeechSynthesis and WebAudio for cues.
Supports import/export JSON and CSV editor input.
*/
(() => {
  // -- Utilities
  ...
  // Web Audio simple tone generator
  ...
  // -- App state
  let state = {
    themes: [], // {id,name,questions: [{questionText,options[],correctIndex,difficulty,source}]}
  ...
  // default minimal themes (so app has something at start)
    ...
    if (localStorage.getItem('quiz_themes')) {
      ...
      state.themes = [
        { id: 'muumit', name: 'Muumit', questions: [
          { questionText: "Kuka on Muumipapan puoliso?", options:["Muumimamma","Niiskuneiti","Pikku Myy"], correctIndex:0, difficulty:'easy', source:'' }
        ]},
        { id: 'hp', name: 'Harry Potter', questions: [
          { questionText: "Missä koulu opettaa velhoja?", options:["Hogwarts","Beauxbatons","Durmstrang"], correctIndex:0, difficulty:'easy', source:'' }
        ]}
      ];
  ...
  function updateScore(){
    scoreArea.textContent = `Pisteet: ${state.score}`;
  }
  ...
  function confirmAnswer(){
    ...
    if (state.selectedOption === q.correctIndex){
      state.score++;
      playCorrect(); speakLocalized('Oikein!','Correct!');
    } else {
      playWrong(); speakLocalized('Väärin. Oikea vastaus oli ', 'Wrong. Correct answer was ');
      const ans = q.options[q.correctIndex];
      speak(ans, languageSelect.value==='fi'?'fi-FI':'en-US');
    }
    // next question
    stopTimer();
  ...
  function startTimerForDifficulty(){
    ...
    // announce initial time maybe
    // start tick interval
    state.timer = setInterval(()=>{
      state.timeRemaining--;
      if (state.timeRemaining <= 3 && state.timeRemaining > 0){
        // audible countdown and tick sound
        playTick();
        const n = state.timeRemaining;
        if (lang==='fi') {
          const words = ['yksi','kaksi','kolme'];
          speak(words[n-1] || String(n),'fi-FI',0.8);
        } else {
          const words = ['one','two','three'];
          speak(words[n-1] || String(n),'en-US',0.8);
        }
    ...
    reader.onload = e => {
      ...
      try {
        ...
        if (parsed.questions && Array.isArray(parsed.questions)){
          // import into themes (group by theme)
      ...
      // try CSV simple lines: theme | question|opt1;opt2;opt3|idx  OR question|opt1;opt2;opt3|idx
      const lines = text.split(/\r?\n/).map(l=>l.trim()).filter(Boolean);
      lines.forEach(line=>{
        const parts = line.split('|').map(p=>p.trim());
        if (parts.length >= 3){
          // if first field matches theme name that exists? We'll assume no theme: use selected theme
  ...
  function playCorrect(){ playCorrectSound(); }
  function playWrong(){ playWrongSound(); }
  function playTick(){ playTickSound(); }

  // alias wrappers with retry to resume audio context if needed
  function playTickSound(){ try{ playTone(1320,0.07,'sine',0.12);}catch(e){} }
  function playCorrectSound(){ try{ playTone(880,0.14,'sine',0.22);}catch(e){} }
  function playWrongSound(){ try{ playTone(220,0.34,'sine',0.22);}catch(e){} }
  ...
  helpBtn.onclick = ()=> {
    ...
    const helpText = lang==='fi' 
      ? 'Ohje: Valitse teema, paina Aloita. Pyyhkäise tai käytä nuolinäppäimiä valitaksesi vaihtoehdon. Paina Vahvista tai Enter. Editorissa voit liittää kysymyksiä muodossa Kysymys|opt1;opt2;opt3|0.'
